...
def _build_common_rules() -> str:
    ...
    "STT 오인식 보정 기준:\n"
    '- STT transcript에는 음성 인식 오류가 포함될 수 있으므로, 문맥상 명확한 기술 용어는 자연스럽게 보정하세요.\n'
    '- 예: "페스트 API", "패스트 API", "Fast API"는 문맥상 FastAPI를 의미하면 "FastAPI"로 보정하세요.\n'
    '- 예: "SKU 라이트", "SKU의 라이트", "에스큐라이트"는 문맥상 데이터베이스를 의미하면 "SQLite"로 보정하세요.\n'
    '- 예: "SQL 알캐미", "SQL 알캐니", "알캐니 ORM", "OARM"은 문맥상 ORM을 의미하면 "SQLAlchemy ORM" 또는 "ORM"으로 보정하세요.\n'
    '- 예: "마이 SKU", "마의 SKU"는 문맥상 데이터베이스를 의미하면 "MySQL"로 보정하세요.\n'
    '- 예: "포스트의 SKU", "포스트그레스"는 문맥상 데이터베이스를 의미하면 "PostgreSQL"로 보정하세요.\n'
    '- 예: "뱀견드", "백앤드"는 문맥상 서버 개발 영역을 의미하면 "백엔드"로 보정하세요.\n'
    '- 예: "CPO", "씨피오"는 문맥상 연산 장치를 의미하면 "CPU"로 보정하세요.\n'
    '- 예: "GPO", "지피오"는 문맥상 그래픽 연산 장치를 의미하면 "GPU"로 보정하세요.\n'
    '- 예: "오시아", "오씨알"은 문맥상 이미지 문자 인식을 의미하면 "OCR"로 보정하세요.\n'
    '- 예: "캡스틱", "캡스톤"은 문맥상 졸업 프로젝트를 의미하면 "캡스톤"으로 보정하세요.\n'
    '- 단, 사람 이름, 담당자 이름, 날짜, 수치, 고유명사는 문맥과 참석자 목록으로 명확히 확인되는 경우에만 보정하세요.\n'
    '- 기술 용어 보정은 summary, decisions, action_items의 task에 반영하되, 확실하지 않으면 원문 의미를 유지하세요.\n'
    "\n"
    ...
    '- "A는 B가 준비한다", "A는 B가 맡는다", "A는 B가 담당한다"처럼 역할 분담이 합의된 문장도 decisions 후보로 보세요.\n'
    ...
    '- STT에서 화자 간의 지시 요청일 경우 담당자 이름을 참석자 목록(attendees)에 있는 이름과 문자열을 정확히 일치시키세요.\n'
    ...
    '- "프론트는 A가 준비", "STT는 B가 준비", "백엔드는 C가 준비"처럼 분야별 담당자가 나뉘는 문장은 각각 별도의 action_items로 분리하세요.\n'
    "\n"
